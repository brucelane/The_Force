...
void main() {
	vec2 uv = (gl_FragCoord.xy / resolution.xy - vec2(0.8,0.005))*2.0;
    ...
    float maxlen = 0.1, dist= 0.0, posx = 0.0, posy = 0.0, posz = 0.0;
    float originx = -bands.x, originy =  bands.x, originz = bands.z;
    ...
    float r = 0.08;//*sin(time*0.25)*sin(time*0.25)+0.1;
    vec3 I = vec3(0.,0.,0.);
    I = sphere( c, r, obs, Lpos, uv, vec3(1.,0.,0.),Lcol, I ) ;
    int numSph = int(time/6.);
